...
import { pilotMessages, pilotEnabled, reservePilot, validatePilotWav } from '@/lib/ai-pilot';
...
function fail(code:string,status=400){return NextResponse.json({code,error:pilotMessages[code]||errors[code]||errors.PROVIDER},{status,headers});}
export async function GET(){try{const client=await createClient(true);const auth=await client?.auth.getUser();if(!auth?.data.user||auth.error)return fail('UNAUTHORIZED',401);const code=process.env.AI_EXECUTION_ENABLED!=='true'?'AI_PAUSED':paidAIAccessError(auth.data.user);return NextResponse.json({enabled:!code,pilot:pilotEnabled(),message:code?(pilotMessages[code]||errors[code]):pilotEnabled()?'Teste económico: apenas WAV PCM de 16 bits até 60 segundos. Reserva de 0,20 € por tentativa, não reembolsada automaticamente. Consulte /setup/pilot.':''},{headers});}catch{return fail('SETUP',503);}}
export async function POST(request:Request){
 ...
 try{
  ...
  const billing=paidAIAccessError(auth.data.user);if(billing)return fail(billing,403);
  ...
  await reservePilot(admin,actor,id,'transcription');
  const openai=new OpenAI({apiKey:process.env.OPENAI_API_KEY,baseURL:'https://api.openai.com/v1',timeout:120000,maxRetries:0});
  const result=await openai.audio.transcriptions.create({file:await toFile(bytes,`audio.${ext}`),model:'gpt-4o-mini-transcribe',response_format:'json',...(language==='auto'?{}:{language})});
  ...
  return NextResponse.json({text,generatedAt:new Date().toISOString()},{headers});
 ...
 const code=error instanceof Error&&(['FORMAT','SIZE','EMPTY'].includes(error.message)||Object.hasOwn(pilotMessages,error.message))?error.message:'PROVIDER';return fail(code,code==='PROVIDER'||code==='EMPTY'?502:400);}
}
