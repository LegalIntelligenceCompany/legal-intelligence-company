...
export async function POST(request:Request){
 ...
 try{
  ...
  const billing=paidAIAccessError();if(billing)return fail(billing,403);
 ...
 const code=error instanceof Error&&['FORMAT','SIZE','EMPTY'].includes(error.message)?error.message:'PROVIDER';return fail(code,code==='PROVIDER'||code==='EMPTY'?502:400);}
}
