...
async function auth(){const client=await createClient(true);const user=(await client?.auth.getUser())?.data.user;if(!user?.email_confirmed_at)throw Error('METER_FORBIDDEN');const db=createAdminClient();if(!db)throw Error('LIVE_DISABLED');return {user,db};}
export async function GET(request:Request){try{
 const {user,db}=await auth();let config;try{config=liveCreditConfig();}catch{return json({enabled:false,wallets:[]});}
 const session=new URL(request.url).searchParams.get('session_id');if(session)await fulfillLiveCheckout(db,session,user.id);
 ...
 return json({enabled:true,wallets,settledOrderIds:(orders.data||[]).map(o=>o.id),organizations:orgs.data||[],terms:config.terms,termsURL:config.termsURL,userId:user.id});
...
export async function POST(request:Request){try{
 ...
 const {user,db}=await auth(),config=liveCreditConfig();
 ...
 if(b.action==='portal')return json({url:await livePortal(db,user.id,b.walletId)});
