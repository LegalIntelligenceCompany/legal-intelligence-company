...
import { meterMessages, meteredResearchBody, recordCommercialResearch, settleCommercialResearch } from '@/lib/commercial-meter';
...
async function handle(request: Request, progress: (stage:string)=>void = ()=>{}) {
  ...
  try {
    ...
    const meter = await reserveService(admin, auth.user, input.requestId, research ? 'assistant-research' : 'document', research ? 'research' : 'document', request);
    const fundedBody = (index:number, body:Record<string,unknown>) => meter ? meteredResearchBody(meter,index,body) : {...body,...pilotOptions()};
    ...
    const raw = await openai.post<Record<string, unknown>, unknown>("/responses", { body: fundedBody(0,{
      model,
    ...
    if(meter) await recordCommercialResearch(admin,auth.user.id,meter,0,raw);
    ...
    const reviewed = await openai.post<Record<string, unknown>, unknown>("/responses", { body: fundedBody(1,{
