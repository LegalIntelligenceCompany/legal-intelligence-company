...
export default async function Page(){
 ...
 const externalEnabled=process.env.AI_EXTERNAL_MODELS_ENABLED==='true';
 const canTest=externalEnabled&&installed&&!trial?.data&&available>=100&&!!process.env.ANTHROPIC_API_KEY&&pilotEnabled()&&process.env.AI_EXECUTION_ENABLED==='true'&&Date.now()<Date.parse(PILOT_EXPIRES)&&Date.now()<Date.parse(budget?.data?.expires_at||'');
 ...
 {!externalEnabled&&<section className="card team-panel"><h2>Apenas OpenAI para clientes</h2><p>Claude e Gemini estão desligados. As integrações, chaves, configurações e resultados foram preservados. Não aparecem na escolha do cliente e novas chamadas de geração estão bloqueadas, incluindo o teste Claude. A reactivação exige ordem do titular, tarifas e validação; não basta ter uma chave.</p></section>}
