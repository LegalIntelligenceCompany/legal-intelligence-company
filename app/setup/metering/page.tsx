...
export default async function Page(){
 ...
 const sql=['013_inference_wallet.sql','014_live_credit_funding.sql','015_audio_metering.sql','016_commercial_pricing.sql'].map(file=>readFileSync(join(process.cwd(),'supabase/migrations',file),'utf8')).join('\n\n');
 ...
 <p>Inclui a actualização 016: novos pedidos a 3,5×, pedidos anteriores preservados a 3× e carregamentos de 20 €, 50 € ou 100 €, antes de IVA. Sem esta actualização, os novos pedidos comerciais ficam bloqueados antes de chamar a IA.</p>
 <details><summary>Carteira já instalada: copiar apenas a actualização de preços</summary><ServicesSetup sql={readFileSync(join(process.cwd(),'supabase/migrations/016_commercial_pricing.sql'),'utf8')}/></details>
 <h2>Instalação completa da carteira</h2><ServicesSetup sql={sql}/><h2>Antes da activação comercial</h2><p>O financiamento está implementado em /credits, mas bloqueado por configuração. É necessário confirmar os preços Stripe de 49 € e 99 € antes de IVA, condições de venda, impostos, webhook live, tarifas versionadas e câmbio. A leitura de subscrições é confirmada na Stripe antes de cada reserva.</p>
