...
export default async function Page(){
 ...
 const sql=['013_inference_wallet.sql','014_live_credit_funding.sql'].map(file=>readFileSync(join(process.cwd(),'supabase/migrations',file),'utf8')).join('\n\n');
 ...
 <p>Esta ligação cobre a pesquisa recuperável do chat. Análises, ferramentas documentais e transcrição continuam limitadas ao piloto; não recebem autorização comercial por esta actualização.</p>
 <Link className="btn btn-secondary" href="/chat">Voltar ao chat</Link></AppShell>;
}
