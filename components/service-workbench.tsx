...
export function ServiceWorkbench({workflow}:{workflow:Workflow}) {
 ...
 if(['timeline','explainer','evidence','dossier-search','negotiation'].includes(workflow)||(workflow==='reviewer'&&privateDocument))return <CompanyWorkspace title={w.title} description={w.description}>{company=><><p>PDF privado: apenas leitura documental, sem confirmação externa de legislação. Disponibilidade e custo confirmados antes do envio.</p><Link href="/credits">Consultar planos e créditos</Link>{toggle}<Link href="/contracts/new">Carregar PDFs</Link><AssistantPanel key={`${workflow}-private`} organizationId={company.id} workflow={workflow}/></>}</CompanyWorkspace>;
 return <AppShell><Link href="/services">Todos os serviços</Link><h1>{w.title}</h1><p>Disponibilidade e custo confirmados antes do envio. Não é possível garantir ausência de erros. Pode guardar respostas públicas num dossier ou exportá-las.</p>{toggle}<AssistantPanel key={`${workflow}-public`} workflow={workflow}/></AppShell>;
}
