...
export function TranscriptionPanel(){
 ...
 function clear(){epoch.current++;stop();request.current?.abort();setFile(null);setText('');setConsent(false);setRecoveryConsent(false);setError('');setRecording(false);setSeconds(0);}
 ...
 return <section className="card team-panel">
  ...
  <p>O áudio fica apenas nesta página até carregar em Transcrever e não é guardado pelo site. Sem autorização de recuperação, o texto perde-se ao sair. O fornecedor tem as suas próprias regras de retenção.</p>
  <label className="analysis-consent"><input type="checkbox" checked={recoveryConsent} disabled={busy||recording} onChange={e=>setRecoveryConsent(e.target.checked)}/>Autorizo guardar o resultado nesta conta para recuperação durante 24 horas. Após expirar fica inacessível; a eliminação ocorre na próxima consulta ou limpeza programada. O áudio não é guardado.</label><Link href="/recoveries">Recuperar resultados sem repetir o pedido</Link>
