...
import {ClaimEvidencePanel} from './claim-evidence';
...
export function AssistantPanel({ organizationId, workflow }: { organizationId?: string; workflow?: Workflow }) {
  ...
  useUnsavedWork(busy||turns.length>0);
  ...
  useEffect(() => {
    ...
    async function load(id: string | null) {
      if (!alive) return;
      if (user.current !== id) { setRecoveryConsent(false); setMaterial(""); epoch.current++; setTurns([]); setQuestion(""); setConsent(false); setDocuments([]); setDocumentA(""); setDocumentB(""); setSelectedDocuments([]); setError(""); setDate(""); setTitle(""); setDateConfirmed(false); }
  ...
  async function send(event: React.FormEvent) {
    ...
    try {
      ...
      const response = await fetch("/api/assistant", { method: "POST",signal:controller.signal, headers: { "Content-Type": "application/json",Accept:'application/x-ndjson','x-recovery-consent':String(recoveryConsent),...funding.headers }, body: JSON.stringify({ requestId: crypto.randomUUID(), mode, profile, country, question: asked, history, consent, organizationId, ...(workflow ? {workflow, format} : {}), ...(mode === "private-text" ? {material} : {}), documentIds: mode === "research" || mode === "private-text" ? [] : multiDocument ? selectedDocuments : mode === "compare" ? [documentA, documentB] : [documentA] }) });
  ...
  return <div className="assistant-layout">
    ...
    <section className="card team-panel assistant-main" aria-label="Conversa com o assistente">
      ...
      {error && <p role="alert" className="workspace-error">{error} <Link className="source-link" href="/help">Ajuda com o pedido</Link></p>}
      {session === "loading" ? <p>A verificar a sessão…</p> : session === "login" ? <p className="team-notice">Entre com o seu e-mail para utilizar IA. Não precisa de empresa para pesquisar direito. <Link className="btn btn-primary" href="/login?next=/chat">Entrar</Link></p> : <><label className="analysis-consent"><input type="checkbox" checked={recoveryConsent} disabled={busy} onChange={e=>setRecoveryConsent(e.target.checked)}/>Autorizo guardar apenas o resultado nesta conta para recuperação durante 24 horas. Após expirar fica inacessível; a eliminação ocorre na próxima consulta ou limpeza programada.</label><Link href="/recoveries">Recuperar resultados sem repetir a geração</Link><form onSubmit={send}>
