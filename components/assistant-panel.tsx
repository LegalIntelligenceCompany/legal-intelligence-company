...
export function AssistantPanel({ organizationId, workflow }: { organizationId?: string; workflow?: Workflow }) {
  const [mode, setMode] = useState<AssistantMode>(workflow === "timeline" ? "timeline" : organizationId ? "document" : "research");
  ...
  useEffect(() => {
    ...
    async function load(id: string | null) {
      if (!alive) return;
      if (user.current !== id) { epoch.current++; setTurns([]); setQuestion(""); setConsent(false); setDocuments([]); setDocumentA(""); setDocumentB(""); setSelectedDocuments([]); setError(""); setDate(""); setTitle(""); setDateConfirmed(false); }
  ...
  async function send(event: React.FormEvent) {
    ...
    try {
      ...
      const response = await fetch("/api/assistant", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ requestId: crypto.randomUUID(), mode, profile, country, question: asked, history, consent, organizationId, ...(workflow ? {workflow, format} : {}), documentIds: mode === "research" ? [] : mode === "timeline" ? selectedDocuments : mode === "compare" ? [documentA, documentB] : [documentA] }) });
  ...
  return <div className="assistant-layout">
    ...
    <section className="card team-panel assistant-main" aria-label="Conversa com o assistente">
      ...
      <div aria-live="polite" aria-busy={busy}>{turns.map((turn, i) => <article className="assistant-turn" key={i}><h3 className="assistant-question">{turn.question}</h3><p className="assistant-small">{turn.result.researched ? "Pesquisa web com citações · vigência a confirmar" : "Baseado nos documentos · sem pesquisa web"} · {new Date(turn.result.generatedAt).toLocaleString("pt-PT")}</p>{turn.result.review === "second-pass" && <p className="assistant-small">{turn.result.model} · Segunda revisão por IA concluída; não equivale a validação por jurista.</p>}<Answer result={turn.result}/>{!organizationId && <SaveResearch question={turn.question} result={turn.result}/>}<button type="button" className="btn btn-secondary" onClick={() => downloadReport(reportText(turn.result), "relatorio-lic.txt")}>Exportar resposta e fontes</button></article>)}{busy && <p role="status" className="team-notice">{mode === "research" ? "A pesquisar fontes, elaborar e rever a resposta…" : "A ler os documentos, elaborar e rever a resposta…"} Pode demorar até três minutos. Não volte a enviar.</p>}</div>
      ...
      {session === "loading" ? <p>A verificar a sessão…</p> : session === "login" ? <p className="team-notice">Entre com o seu e-mail para utilizar IA. Não precisa de empresa para pesquisar direito. <Link className="btn btn-primary" href="/login?next=/chat">Entrar</Link></p> : <form onSubmit={send}>
        ...
        <button className="btn btn-primary" disabled={busy || !consent || !question.trim() || (!!organizationId && (mode === "timeline" ? selectedDocuments.length === 0 : (!documentA || (mode === "compare" && (!documentB || documentB === documentA)))))}>{busy ? "A preparar…" : "Enviar pergunta"}</button>
      </form>}
