...
export function LiveCreditsPanel(){
 ...
 {!state?<p>A carregar a carteira…</p>:!state.enabled?<><p role="status">Novas compras estão desligadas. Pode consultar os saldos existentes e gerir ou cancelar uma subscrição anterior.</p>{state.wallets.map(w=><article className="card" key={w.id}><h2>{w.scope==='company'?'Carteira da empresa':'Carteira pessoal'}</h2><p>Disponível: {euros(w.availableCents)} · Reservado: {euros(w.reservedCents)} · Saldo total: {euros(w.balanceCents)}</p>{w.isOwner&&w.subscribed&&<button className="btn btn-secondary" disabled={busy} onClick={()=>void action({action:'portal',walletId:w.id})}>Gerir ou cancelar subscrição</button>}</article>)}<Link href="/usage">Histórico de consumo</Link></>:<>
 ...
 <label>Carregamento de créditos antes de IVA<select value={amount} onChange={e=>setAmount(e.target.value)} disabled={busy}><option value="20">20 €</option><option value="50">50 €</option><option value="100">100 €</option></select></label><p>Compra única, sem carregamentos automáticos. O total, com os impostos aplicáveis, é apresentado na Stripe antes da confirmação.</p>
