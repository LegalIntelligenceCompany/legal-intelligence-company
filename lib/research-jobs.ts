...
export type ResearchJob={id:string;owner_id:string;input:AssistantInput;model:string;state:string;funding_mode?:'pilot'|'commercial';response_id?:string;result?:AssistantResult;error?:string;created_at?:string;updated_at:string;expires_at:string};
...
export function publicJob(job:ResearchJob){return {id:job.id,question:job.input.question,state:job.state,model:job.model,createdAt:job.created_at,updatedAt:job.updated_at,expiresAt:job.expires_at,result:job.state==='completed'?job.result:undefined,error:job.error?researchErrors[job.error]||job.error:undefined};}
