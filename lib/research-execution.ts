...
import {measuredResearchModel} from './model-routing';
import {collectOriginals} from './research-originals';
...
export async function GET(request:Request){
 ...
 const automatic=measuredResearchModel(process.env.AI_MODEL_EVALUATIONS_JSON);
 if(automatic&&commercialMeterEnabled()&&deniedCommercial(user)){
  try{const ceiling=meterConfiguration(automatic===PILOT_MODEL?'economical':'advanced').ceiling;models.push({id:'auto',label:'Automático avaliado · '+automatic,available:true,ceiling});}catch{/* No validated tariff: do not offer routing. */}
 }
 return json({job:found.data?publicJob(found.data):null,funding,models,backgroundNotifications:!!process.env.OPENAI_WEBHOOK_SECRET});
}
...
export async function POST(request:Request){
 ...
 return executeResearch(body,await context());
}
// HTTP callers use their session; workers resolve the persisted owner in Auth.
export async function executeResearch(body:Record<string,unknown>,ctx:{user:User|null|undefined;admin:ReturnType<typeof createAdminClient>}){
 const {user,admin}=ctx;if(!user||!admin)return fail('FORBIDDEN',401);
 ...
 try{
  ...
  if(body.action==='start'){
   ...
   const model=body.model==='auto'?measuredResearchModel(process.env.AI_MODEL_EVALUATIONS_JSON):body.model??PILOT_MODEL;
   if(body.model==='auto'&&(!model||!commercial))return fail('MODEL_UNAVAILABLE',403);
  ...
  if(job!.state==='draft'){
   ...
   if(job!.model!==PILOT_MODEL)draft.originals=await collectOriginals(draft);
