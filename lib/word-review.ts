...
function zip(files:Record<string,string>){const locals:Uint8Array[]=[],directory:Uint8Array[]=[];let offset=0;for(const [name,text] of Object.entries(files)){const path=encoder.encode(name),data=encoder.encode(text),crc=crc32(data);const local=header(30,[[0,4,0x04034b50],[4,2,20],[12,2,33],[14,4,crc],[18,4,data.length],[22,4,data.length],[26,2,path.length]]);locals.push(local,path,data);directory.push(header(46,[[0,4,0x02014b50],[4,2,20],[6,2,20],[14,2,33],[16,4,crc],[20,4,data.length],[24,4,data.length],[28,2,path.length],[42,4,offset]]),path);offset+=local.length+path.length+data.length;}
