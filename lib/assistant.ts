...
export type OriginalEvidence = {sourceId:number;url:string;title:string;text:string;retrievedAt:string;sha256:string;truncated:boolean};
export type ClaimEvidence = {block:number;heading:string;sourceId:number;url:string;quote:string;literalMatch:boolean;assessment:'supports'|'contradicts'|'insufficient'};
export type AssistantResult = { text: string; citations: Citation[]; researched: boolean; generatedAt: string; review?: "second-pass"; model?: string; chargedCents?: number; originals?:OriginalEvidence[]; evidence?:ClaimEvidence[] };
