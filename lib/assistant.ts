...
export const modes = ["research", "document", "compare", "obligations", "timeline"] as const;
...
export type AssistantInput = { requestId: string; mode: AssistantMode; profile: typeof profiles[number]; country: "Portugal" | "União Europeia"; question: string; history: { role: "user" | "assistant"; content: string }[]; documentIds: string[]; organizationId?: string; consent: true; workflow?: Workflow; format?: string };
...
export function validateAssistantInput(value: unknown): AssistantInput {
  ...
  if (v.mode === "timeline" && (count < 1 || count > 5)) throw new Error("INVALID_REQUEST");
...
export function assistantInstructions(input: AssistantInput) {
  const service = input.workflow ? workflowInstructions(input.workflow, input.format!) : "";
...
${input.mode === "research" ? `Pesquisa fontes primárias oficiais: Diário da República, DGSI, tribunais, EUR-Lex e CURIA. Cita fontes junto das afirmações. Para legislação verifica versão, alterações, entrada em vigor e âmbito temporal; se não conseguires, diz expressamente 'vigência não confirmada'. Para acórdãos identifica tribunal, data e processo apenas quando constem da fonte, e distingue decisão de norma vinculativa. Explica limites de acesso e conclusões divergentes. Não solicites dados pessoais nem contratos confidenciais. Usa texto claro com títulos curtos. As pesquisas externas podem conter a pergunta e contexto.` : `Não tens pesquisa web neste modo. Não afirmes verificar legislação vigente. Os ficheiros são Documento A${input.mode === "timeline" ? " até Documento " + String.fromCharCode(64 + input.documentIds.length) : input.mode === "compare" ? " e Documento B (versão posterior)" : ""}. Baseia a resposta exclusivamente nos ficheiros, com excertos breves e página quando identificável. Declara partes ilegíveis ou ausentes. Não trates citações como verificadas automaticamente.
